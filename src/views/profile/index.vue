...
<template>
    <main>
        ...
        <div class="flex justify-center">
            <div class="w-full max-w-960px flex flex-row flex-wrap">
                ...
                <div class="ml-96 mt-10">
                    <div class="mt-10" v-for="order in orders">
                        <div>
                            <span>
                                <Button label="支付" @click="checkout_visible = true" />

                                <Dialog v-model:visible="checkout_visible" modal header="支付订单"
                                    :style="{ width: '25rem' }">
                                    ...
                                    <div class="mt-2 flex justify-content-end gap-2">
                                        <Button type="button" label="支付" @click="checkout(order.id)"></Button>
                                    </div>
                                </Dialog>
                            </span>
                            <span class="ml-2"><Button type="button" label="删除"
                                    @click="cancel_order(order.id)"></Button></span>
                        </div>
                        <span>订单id: {{ order.id }}</span>
                        <div>订单状态: {{ order.status }}</div>
                        <div>下单日期: {{ order.date }}</div>
                        <div>收货地址: {{ order.address }}</div>
                        <Button type="button" @click="address_visible = true" label="修改地址"></Button>
                        <Dialog v-model:visible="address_visible" modal header="修改地址" :style="{ width: '25rem' }">
                            ...
                            <div class="mt-2 flex justify-content-end gap-2">
                                <Button type="button" label="修改" @click="modify_address(order.id)"></Button>
                            </div>
                        </Dialog>
